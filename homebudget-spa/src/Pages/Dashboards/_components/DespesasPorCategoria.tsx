...
export default function DespesasPorCategoria() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const url = `${apiUrl}/relatorios/despesas-por-categoria?anoInicio=${anoInicio}&mesInicio=${mesInicio}&anoFim=${anoFim}&mesFim=${mesFim}`;

      const response = await axios.get(url, {
        headers: { Authorization: `Bearer ${token}` },
      });

      const transactions: DespesasPorCategoriaData[] = response.data;
  ...
  return (
    <div className={(styles.graficos, styles.card)}>
      <h4 className={styles.card_title}>Despesas por Categoria</h4>
